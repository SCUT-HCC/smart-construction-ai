...
| 获取项目真实架构框架 | `docs/CODEMAPS/` | 代码地图：架构、模块、数据流、依赖 |
| 查看章节结构分析报告 | `docs/analysis/` | 16份文档的章节分析，含建议10章节结构 |
| 了解生成系统设计方案 | `docs/generation_system_design.md` | 施工方案生成系统MVP设计 |
| 了解施工方案模板与规范 | `templates/` | 10章节标准模板、命名规范 |
| 查看清洗后的数据示例 | `output/1-16/` | 16份清洗后的样本文档（final.md） |
...
1. **章节完整性检查**
   - 建议 10 章节结构（基于16份实际文档分析）：
     ...
     9. 应急预案与处置措施
     10. 绿色施工与环境保护（可选）✨
   - 允许标题灵活命名，但内容必须覆盖核心章节
   - 输出：缺失章节清单 + 完整性评分
   - 详见：`docs/analysis/chapter_comparison_table.md`
...
3. 基于 `templates/standard_50502.md` 的 10 章节结构生成完整方案
4. 调用知识库填充内容
5. 风险点识别与保障措施生成

> [!NOTE]
> 章节命名必须严格遵守 `templates/naming_conventions.md` 中的规范
...
| Phase 2 | 2026-03 | 知识库构建（qmd + LightRAG） |
